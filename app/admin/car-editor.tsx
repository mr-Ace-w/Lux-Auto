...
export function CarEditor({car}:{car?:any}){
  ...
  async function files(list:FileList|null){
    if(!list)return;
    if(images.length+list.length>50)return alert('Максимум 50 фотографій');
    setBusy(true);
    try{
      ...
      for(const file of Array.from(list)){
        ...
        if(!res.ok){
          ...
          throw new Error(err.error||'Помилка завантаження фото в Cloudflare R2');
        }
        const data=await res.json();
        uploaded.push({id:crypto.randomUUID(),url:data.url,position:images.length+uploaded.length});
      }
  ...
  async function removeOrphanedFiles(){
    const removedUrls=initialImages.map(image=>image.url).filter(url=>!images.some(current=>current.url===url));
    if(removedUrls.length){
      try{
        await fetch('/api/upload/delete',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({urls:removedUrls})});
      }catch(e){
  ...
  async function save(e:React.FormEvent){e.preventDefault();setBusy(true);try{const details=Object.fromEntries(form.details.split('\n').filter(Boolean).map((line:string)=>{const [k,v]=line.split(':',2);return [k?k.trim():'',v?v.trim():'']}).filter((item:string[])=>item[0]));const trimmedForm:any={};Object.keys(form).forEach(key=>{if(typeof form[key]==='string'){trimmedForm[key]=form[key].trim()}else{trimmedForm[key]=form[key]}});const row={...trimmedForm,fuel:normalizeFuel(trimmedForm.fuel),year:+form.year,price:+form.price,mileage:+form.mileage,features:form.features.split('\n').filter(Boolean).map((f:string)=>f.trim()).filter(Boolean),details};const result=car?await db.from('cars').update(row).eq('id',car.id).select().single():await db.from('cars').insert(row).select().single();if(result.error)throw result.error;const removed=await db.from('car_images').delete().eq('car_id',result.data.id);if(removed.error)throw removed.error;if(images.length){const inserted=await db.from('car_images').insert(images.map((image,position)=>({car_id:result.data.id,url:image.url,position})));if(inserted.error)throw inserted.error}await removeOrphanedFiles();router.replace('/admin');router.refresh()}catch(error:any){alert(error.message);setBusy(false)}}
  return (
