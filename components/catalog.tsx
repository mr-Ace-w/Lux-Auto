...
export function Catalog({ cars: rawCars }: { cars: Car[] }) {
  ...
  const fuelOptions = useMemo(() => {
    const rawFuels = cars.map((car) => car.fuel).filter(Boolean);
    const set = new Set<string>();
    rawFuels.forEach((fuel) => {
      set.add(fuel);
      if (fuel.includes('/')) {
        fuel.split('/').forEach((part) => {
  ...
  useEffect(() => {
    setLimit(16);
  }, [selectedBrands, selectedModels, priceFrom, priceTo, yearFrom, yearTo, mileageFrom, mileageTo, selectedFuels, selectedTransmissions, selectedTransportType, sort]);
  ...
  }, [sort, selectedBrands, selectedModels, priceFrom, priceTo, yearFrom, yearTo, mileageFrom, mileageTo, selectedFuels, selectedTransmissions, selectedTransportType, limit]);
  ...
  }, [selectedBrands, selectedModels, cars, selectedFuels, selectedTransmissions, selectedTransportType, mileageFrom, mileageTo, priceFrom, priceTo, sort, yearFrom, yearTo]);
                ...
                <div className="dropdown-menu transport-type-menu">
                  <div className="options-list">
                    {transportTypeOptions.map(opt => (
                      <label key={opt} className="option-radio-label">
                        <input 
                          type="radio" 
                          name="transportTypeOption"
                          checked={tempTransportType === opt} 
                          onChange={() => setTempTransportType(opt)}
                        />
                        <span>{opt}</span>
                      </label>
                    ))}
